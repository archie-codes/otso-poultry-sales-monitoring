...
// import { Tractor, Building2, Egg } from "lucide-react";

// // Modals and Buttons
// import AddFarmModal from "./AddFarmModal";
// import AddBuildingModal from "./AddBuildingModal";
// import DeleteAlertButton from "./DeleteAlertButton"; // <-- Imported the new Safe Delete button!
...
//                       <div
//                         key={building.id}
//                         // NEW: Added justify-between to push the delete button to the right edge
//                         className="flex items-center justify-between p-3 rounded-xl border border-border/40 bg-background/50 transition-colors"
//                       >
//                         {/* Wrapper for the Icon and Text */}
...
//                         {/* NEW: Safe Delete Button for the Building */}
//                         <DeleteAlertButton
//                           id={building.id}
//                           name={building.name}
//                           type="building"
...
export default async function FarmsPage() {
  ...
  // 1. Fetch Farms and Buildings
  const allFarms = await db.select().from(farms);
  const allBuildings = await db.select().from(buildings);

  // 2. NEW: Fetch only active loads to see which buildings are occupied
  const activeLoads = await db
    .select({ buildingId: loads.buildingId })
    .from(loads)
    .where(eq(loads.isActive, true));

  // Create a quick lookup Set for active building IDs
  const activeBuildingIds = new Set(activeLoads.map((l) => l.buildingId));

  // 3. Attach the status to the buildings array
  const buildingsWithStatus = allBuildings.map((b) => ({
    ...b,
    hasActiveLoad: activeBuildingIds.has(b.id),
  }));

  // Pass the updated buildings array to the client
  return (
    <FarmsClient
      initialFarms={allFarms}
      initialBuildings={buildingsWithStatus}
    />
  );
}
