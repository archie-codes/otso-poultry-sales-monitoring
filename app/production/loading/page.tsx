...
//       .reduce((sum, fa) => sum + fa.remainingInBuilding, 0); // We sum up whatever is actually left in the building!
...
export default async function LoadingPage() {
  ...
  const activeLoads = activeLoadsRaw.map((load) => {
    ...
    // --- Feed Inventory Math (Upgraded to Two-Tier!) ---
    // FIX: Safely wrap remainingInBuilding in Number() to satisfy TypeScript
    const remainingFeeds = allBuildingFeeds
      .filter((fa) => fa.loadId === load.id)
