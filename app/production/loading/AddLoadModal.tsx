...
// //         createPortal(
// //           <div className="fixed inset-0 z-100 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
// //             {/* ---> THE UI FIX: flex-col, overflow-hidden <--- */}
...
//     // ---> THE FIX: FORCE UPPERCASE BEFORE SAVING TO DATABASE <---
//     formData.set("name", batchName.toUpperCase().trim());

//     // Also explicitly uppercase customer name if it exists to keep DB clean
...
export default function AddLoadModal({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // ---> STRICT VALIDATION ON SUBMIT: Enforce Cleaning Period <---
    if (loadDate && selectedBuilding?.lastHarvestDate) {
      ...
      if (newLoadD.getTime() <= lastHarvest.getTime()) {
        toast.error("Building Needs Cleaning!", {
          description: `This building was harvested on ${format(lastHarvest, "MMM d, yyyy")}. You must load chicks AFTER this date.`,
          style: { backgroundColor: "red", color: "white", border: "none" },
        });
        return;
      }
                ...
                >
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <div className="space-y-2">
                        ...
                        >
                          <Calendar
                            ...
                            disabled={(date) => {
                              ...
                              // ---> THE FIX: BLOCK DATES ON OR BEFORE LAST HARVEST <---
                              if (selectedBuilding?.lastHarvestDate) {
                                ...
                                if (
                                  checkDate.getTime() <= lastHarvest.getTime()
                                )
                                  return true;
                              }
                      ...
                      {/* HELPER TEXT SHOWING WHEN IT WAS LAST HARVESTED */}
                      {selectedBuilding?.lastHarvestDate && (
