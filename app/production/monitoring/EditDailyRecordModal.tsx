...
// ---> THE FIX: Removed the static string array! <---

export default function EditDailyRecordModal({
  record,
  activeLoads = [], // ---> THE FIX: Accept the active loads data! <---
}: {
  record: any;
  activeLoads?: any[];
}) {
  ...
  // ---> THE FIX: Calculate Available Feed Types Dynamically! <---
  const selectedLoad = activeLoads.find(
    (l) => String(l.id) === String(record.loadId),
  );

  const availableFeedTypes = useMemo(() => {
    // Start with the feed type currently saved on this record (if any)
    // We add this because if they edit a record where the stock was subsequently
    // depleted to 0, we still want them to be able to see and keep the original feed type!
    const options = new Map<string, number>();
    if (record.feedType) {
      options.set(record.feedType, 0);
    }

    if (selectedLoad && selectedLoad.feedStock) {
      Object.entries(selectedLoad.feedStock).forEach(
        ([type, data]: [string, any]) => {
          if (data.qty > 0 || type === record.feedType) {
            options.set(type, data.qty);
          }
        },
      );
    }

    return Array.from(options.entries()).map(([type, qty]) => ({ type, qty }));
  }, [selectedLoad, record.feedType]);
                    ...
                    <div className="flex justify-between items-center pb-2 border-b border-amber-200/30">
                      ...
                      >
                        ...
                        <PopoverContent className="p-0 z-200 w-48" align="end">
                          <Command>
                            <CommandList>
                              {/* ---> THE FIX: Display the dynamically generated feed types! <--- */}
                              <CommandEmpty>
                                No stock found in this building.
                              </CommandEmpty>
