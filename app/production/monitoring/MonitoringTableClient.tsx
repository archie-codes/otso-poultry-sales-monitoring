...
export default function MonitoringTableClient({
  ...
  const generatePDF = () => {
    ...
    let reportTitle = "Official Daily Monitoring Logs";
    if (activeTab === "mortality")
      reportTitle = "Official Daily Mortality Logs";
    if (activeTab === "feeds")
      reportTitle = "Official Daily Feed Consumption Logs";
    ...
    const tableColumn = ["Date", "Location & Batch"];

    if (activeTab === "all" || activeTab === "mortality") {
      tableColumn.push("Mortality AM", "Mortality PM", "Total Dead");
    }
    if (activeTab === "all" || activeTab === "feeds") {
      tableColumn.push("Feeds AM", "Feeds PM", "Total Feeds", "Feed Type");
    }
    tableColumn.push("Recorded By");

    const tableRows: any[] = [];
    let totalMortalityAm = 0;
    let totalMortalityPm = 0;
    let totalMortalitySum = 0;
    let totalFeedsAm = 0;
    let totalFeedsPm = 0;
    let totalFeedsSum = 0;

    const pdfData = getItemsForPDF();

    pdfData.forEach((r) => {
      const mortAm = Number(r.mortalityAm) || 0;
      const mortPm = Number(r.mortalityPm) || 0;
      const mortTotal = Number(r.mortality) || 0;

      const feedAm = Number(r.feedsAm) || 0;
      const feedPm = Number(r.feedsPm) || 0;
      const feedTotal = Number(r.feeds) || 0;

      totalMortalityAm += mortAm;
      totalMortalityPm += mortPm;
      totalMortalitySum += mortTotal;
      totalFeedsAm += feedAm;
      totalFeedsPm += feedPm;
      totalFeedsSum += feedTotal;
      ...
      if (activeTab === "all" || activeTab === "mortality") {
        rowData.push(String(mortAm), String(mortPm), String(mortTotal));
      }
      if (activeTab === "all" || activeTab === "feeds") {
        rowData.push(
          formatSacks(feedAm),
          formatSacks(feedPm),
          formatSacks(feedTotal),
          r.feedType || "-",
        );
      }

      rowData.push(r.staffName || "System");
      tableRows.push(rowData);
    });

    const footRow: string[] = ["GRAND TOTALS", ""];

    if (activeTab === "all" || activeTab === "mortality") {
      footRow.push(
        String(totalMortalityAm),
        String(totalMortalityPm),
        String(totalMortalitySum),
      );
    }
    if (activeTab === "all" || activeTab === "feeds") {
      footRow.push(
        formatSacks(totalFeedsAm),
        formatSacks(totalFeedsPm),
    ...
    autoTable(doc, {
      ...
      body: tableRows,
      foot: [footRow],
      showFoot: "lastPage", // <--- THE FIX: Prints Grand Total ONLY on the last page!
      theme: "grid",
    ...
    let fileName = "Otso_Daily_Monitoring";
    if (activeTab === "mortality") fileName = "Otso_Mortality_Log";
    if (activeTab === "feeds") fileName = "Otso_Feed_Consumption_Log";
        ...
        <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-border/50 w-full">
              ...
              <PopoverTrigger asChild>
                <Button
                  ...
                  className={cn(
                    ...
                    !selectedDateParam && "text-slate-500 dark:text-slate-400",
                  )}
                >
          ...
          {hasActiveFilters && (
            <Button
              ...
              className="h-10 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-red-500 hover:text-red-600 hover:bg-red-50 transition-colors ml-auto lg:ml-0"
            >
              ...
              filteredHistory.map((record) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-[10px] font-black uppercase text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/30 px-2.5 py-1.5 rounded-lg border border-blue-200/50 flex items-center w-fit">
                      {record.farmName}{" "}
                          ...
                          <div className="flex flex-col items-center gap-1 mt-1.5">
                            ...
                            {Number(record.totalCost) > 0 && (
                              <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100/50 dark:border-emerald-900/30 px-2.5 py-0.5 rounded-md mt-0.5 shadow-sm">
                                Cost: ₱{formatMoney(Number(record.totalCost))}
                              </span>
                            )}
                          </div>
                        </div>
