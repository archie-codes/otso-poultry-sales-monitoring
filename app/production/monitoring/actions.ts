...
import {
  ...
  users,
  feedAllocations,
  feedDeliveries, // <--- IMPORTED FOR UNIT PRICE LOOKUP
  loads,
  buildings,
  expenses, // <--- IMPORTED TO LOG THE COST
} from "../../../src/db/schema";
import { eq, and, desc, asc, like } from "drizzle-orm"; // <--- ADDED 'like'
import { revalidatePath } from "next/cache";
import { getServerSession } from "next-auth";
import { authOptions } from "../../../lib/auth";
...
export async function addDailyRecord(formData: FormData) {
  ...
  try {
    ...
    if (totalFeedsConsumed > 0 && feedType) {
      ...
      // Join allocations with deliveries to get the EXACT unit price
      const allocations = await db
        .select({
      ...
      for (const alloc of allocations) {
        ...
        if (currentRemaining > 0) {
          ...
          // Accumulate the exact financial cost based on where these sacks came from
          totalCost += deductAmount * Number(alloc.unitPrice);
        }
      }
