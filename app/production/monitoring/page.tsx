...
//           if (type) {
//             if (!acc[type]) {
//               // ---> THE FIX: Save the combined price (Unit Price + Bond) <---
//               acc[type] = {
//                 qty: 0,
//                 price: (Number(fa.unitPrice) || 0) + (Number(fa.cashBond) || 0),
//               };
//             }
//             acc[type].qty += Number(fa.remainingInBuilding);
//           }
...
//     })
//     .from(expenses)
//     .where(like(expenses.remarks, "[DailyRecord:%"));

//   const historyWithCost = history.map((h) => {
//     const exp = relatedExpenses.find((e) =>
//       e.remarks?.includes(`[DailyRecord:${h.id}]`),
//     );
//     return {
//       ...h,
//       totalCost: exp ? Number(exp.amount) : 0,
//     };
//   });
...
//         buildings={filteredBuildings}
//         loads={availableLoads}
//         activeLoads={activeLoads} // <--- THE FIX: Passes full inventory to the table!
//         totalPages={totalPages}
//         currentPage={currentPage}
//         userRole={userRole}
//       />
...
export default async function DailyMonitoringPage(props: {
  ...
  // ---> THE FIX: FULL HISTORY (FOR PDF) <---
  const fullHistoryRaw = await db
    .select({
      id: dailyRecords.id,
      loadId: dailyRecords.loadId,
      loadName: loads.name,
  ...
  const historyWithCost = attachCosts(historyRaw);
  const fullHistoryWithCost = attachCosts(fullHistoryRaw);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 max-w-7xl mx-auto pb-12">
      ...
      <MonitoringTableClient
        history={historyWithCost}
        fullHistory={fullHistoryWithCost} // <--- THE FIX: Passing the full data for the PDF!
        farms={uniqueFarmNames}
        buildings={filteredBuildings}
        loads={availableLoads}
