...
export default async function HistoryPage() {
  ...
  // 1. Fetch ONLY INACTIVE (Completed) Loads
  const historicalLoadsData = await db
    ...
    .where(eq(loads.isActive, false)) // STRICTLY FALSE
    .orderBy(
      asc(farms.name),
      asc(buildings.name),
      desc(loads.harvestDate), // Newest completed batches first
    );
  ...
  const reports = historicalLoadsData.map((load) => {
    ...
    const farmActiveLoadsCount =
      historicalLoadsData.filter((l) => l.farmId === load.farmId && !l.isActive)
        .length || 1;
    ...
    // Cost per Chick based on actual harvested birds
