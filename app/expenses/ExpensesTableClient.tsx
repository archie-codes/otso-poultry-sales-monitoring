...
export default function ExpensesTableClient({
  ...
  const generatePDF = () => {
    ...
    fullHistory.forEach((r) => {
      ...
      // ---> THE FIX: FORCE A NEWLINE FOR INITIAL LOAD DETAILS <---
      let cleanRemarks = r.remarks
        ? r.remarks.replace(/±/g, "PHP ").replace(/\+=/g, "PHP ")
        : "-";

      // Looks for "Initial Load:" and dynamically inserts a newline (\n) right after it
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        1: { cellWidth: 80 },
        2: { cellWidth: 25 },
        3: { cellWidth: 75 }, // Details column wraps perfectly
        4: { cellWidth: 30 },
  ...
  const downloadCSV = () => {
    ...
    fullHistory.forEach((r) => {
      ...
      // Ensure CSV honors the multi-line Initial Load formatting (Excel will read \n via alt+enter)
      cleanRemarks = cleanRemarks.replace(/(Initial Load:)\s*/i, "$1\n");
            ...
            {history.length === 0 ? (
              ...
              history.map((record) => (
                ...
                >
                  ...
                  {/* ---> THE FIX: Added whitespace-pre-wrap so the frontend table honors the \n <--- */}
