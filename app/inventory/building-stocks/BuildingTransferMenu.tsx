...
export default function BuildingTransferMenu({
  ...
  // Track the typed quantity to run real-time checks
  const [transferQty, setTransferQty] = useState<string>("");
  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        {/* ---> THE FIX: Added flex col and max-h-[90vh] so the modal doesn't push off screen <--- */}
        <DialogContent className="max-w-md rounded-[2rem] p-0 overflow-hidden border-border/50 flex flex-col max-h-[90vh]">
          {/* Header (Shrink-0 so it stays at top) */}
          <div className="bg-amber-600 p-6 text-white flex items-center gap-4 shrink-0">
            <div className="p-3 bg-amber-500/50 rounded-2xl shrink-0">
              <ArrowRightLeft className="w-6 h-6" />
            </div>
            <div>
          ...
          >
            {/* ---> THE FIX: Scrollable middle area! <--- */}
            <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="space-y-1.5">
                ...
                <Popover open={openFeed} onOpenChange={setOpenFeed}>
                  <PopoverTrigger asChild>
                    {/* ---> THE FIX: min-h-[44px] h-auto whitespace-normal <--- */}
                    <Button
                      variant="outline"
                      role="combobox"
                      aria-expanded={openFeed}
                      className={cn(
                        "w-full min-h-[44px] h-auto py-2 flex items-center justify-between rounded-xl border-border bg-slate-50 dark:bg-slate-900 text-sm font-bold uppercase hover:bg-slate-100 dark:hover:bg-slate-800 whitespace-normal text-left",
                        !selectedFeedId && "text-muted-foreground",
                      )}
                    >
                        ...
                        <CommandGroup>
                          {activeStocks.map((stock) => (
                            ...
                            >
                              <Check
                                className={cn(
                                  "mr-2 h-4 w-4 text-amber-600 shrink-0",
                                  selectedFeedId === String(stock.id)
                                    ? "opacity-100"
                                    : "opacity-0",
                                )}
            ...
            {/* ---> THE FIX: Fixed Sticky Footer (shrink-0) <--- */}
            <div className="flex justify-end gap-3 p-6 border-t border-border/50 bg-slate-50 dark:bg-slate-900/50 shrink-0">
              <Button
                type="button"
                variant="ghost"
                onClick={() => setIsOpen(false)}
